...
async function fetchAllData(){
  ...
  try{
    ...
    if(error)throw error;
    state.properties=(props||[]).map(p=>({id:p.id,name:p.name||'',city:p.city||'',phase:p.phase||'fase0',days:p.days_elapsed||0,units:p.units||0,active_users:p.active_users||0,modules:Array.isArray(p.active_modules)?p.active_modules:[],module_values:(p.module_values&&typeof p.module_values==='object')?p.module_values:{},notes:p.notes||'',core_notes:p.core_notes||'',core_list:p.core_list||'',entry_date:p.entry_date||''}));
...
function openAddModal(){
  ...
  $('fDays').value='0';$('fUnits').value='50';$('fActiveUsers').value='0';$('fNotes').value='';$('fCoreNotes').value='';$('fCoreList').value='';
...
function openEditModal(id){
  ...
  $('fDays').value=p.days||0;$('fUnits').value=p.units||0;$('fActiveUsers').value=p.active_users||0;$('fEntryDate').value=p.entry_date||'';
...
async function saveProperty(){
  ...
  const prop={name,city:$('fCity').value.trim(),phase:$('fPhase').value,days:parseInt($('fDays').value)||0,units:parseInt($('fUnits').value)||0,active_users:parseInt($('fActiveUsers').value)||0,entry_date:entryDate,notes:$('fNotes').value.trim(),core_notes:$('fCoreNotes').value.trim(),core_list:$('fCoreList').value.trim(),modules,module_values};
  closeModal();
  if(editId){
